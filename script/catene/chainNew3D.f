...
c includes image flags in data file so chains can be unraveled later
      ...
      integer swaptype,ichain,jchain,zchain
      integer, allocatable :: nchain(:),nmonomer(:),Nx(:),Ny(:),Nz(:)
...
c setup box size (sigma = 1.0)
      ...
      xprd = 2*nmonomer(1)*bondlength(1)*Nx(1)
      yprd = 2*nmonomer(1)*bondlength(1)*Ny(1)
      zprd = 2*nmonomer(1)*bondlength(1)*Nz(1)
...
c loop over sets and chains in each set
      ...
      do iset = 1,nsets
        do ichain = 1,int(Nx(iset))
           do jchain = 1,int(Ny(iset))
           	  do zchain = 1,int(Nz(iset))
...
c random starting point for the chain in the box
              ...
              y1 = 0.0
              z1 = 0.0
              x2 = xboundlo + 2*nmonomer(1)*bondlength(1)*(ichain - 0.5)
              y2 = yboundlo + 2*nmonomer(1)*bondlength(1)*(jchain - 0.5)
              z2 = zboundlo + 2*nmonomer(1)*bondlength(1)*(zchain - 0.5)
