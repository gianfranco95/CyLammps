...
c includes image flags in data file so chains can be unraveled later
      ...
      integer swaptype, cubic, xconfined, yconfined, zconfined
...
c check that the monomer is created inside the box in cased of fixed boundaries

	    if(xconfined == 1) then	
	    	if (x2 >= xboundhi .OR. x2 <= xboundlo) then
			 goto 10
	        end if
	    end if

	    if(yconfined == 1) then	
	    	if (y2 >= yboundhi .OR. y2 <= xboundlo) then
			 goto 10
	        end if
	    end if

	    if(zconfined == 1) then	
	    	if (z2 >= zboundhi .OR. z2 <= zboundlo) then
			 goto 10
	        end if
	    end if
